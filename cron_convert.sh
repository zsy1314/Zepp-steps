...
function persist_execute_log {
  ...
  current_cron=$(< .github/workflows/main.yml grep cron|awk '{print substr($0, index($0,$3))}')
  {
    echo "current cron:"
  ...
  "${sed_prefix[@]}" -E "s/(- cron: ')[0-9]+( [^[:space:]]+ \* \* \*')/\1$((RANDOM % 59)) ${cron_hours} * * *'/g" .github/workflows/main.yml
