...
def post_fake_brand_data(step, app_token, userid):
    ...
    # 使用 JSON 解析替代正则替换，确保各时段步数与总数一致
    decoded = urllib.parse.unquote(data_json)
    data = json.loads(decoded)

    # 解析内层 summary JSON
    summary = json.loads(data[0]['summary'])
    old_ttl = summary['stp']['ttl']
    target = int(step)
    ratio = target / old_ttl

    # 按比例缩放各时段步数、距离、卡路里
    for stage in summary['stp']['stage']:
        stage['step'] = round(stage['step'] * ratio)
        stage['dis'] = round(stage['dis'] * ratio)
        stage['cal'] = round(stage['cal'] * ratio)

    # 更新总计
    summary['stp']['ttl'] = target
    summary['stp']['dis'] = round(summary['stp']['dis'] * ratio)
    summary['stp']['cal'] = round(summary['stp']['cal'] * ratio)
    if 'runDist' in summary['stp']:
        summary['stp']['runDist'] = round(summary['stp']['runDist'] * ratio)
    if 'runCal' in summary['stp']:
        summary['stp']['runCal'] = round(summary['stp']['runCal'] * ratio)

    # 更新日期
    data[0]['date'] = today

    # 重新序列化 summary
    data[0]['summary'] = json.dumps(summary, separators=(',', ':'))

    # 重新序列化并恢复斜杠转义（保持与原始格式一致）
